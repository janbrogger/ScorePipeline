...
qisMultipleEventPerEEG = ScoreQueryRun(['SELECT SearchResult.MultipleEventsPerEEG FROM SearchResult WHERE SearchResult.SearchResultId =' num2str(handles.SearchResultId)]);
isMultipleEventPerEEG = qisMultipleEventPerEEG{1,1};
setappdata(0,'isMultipleEventPerEEG',isMultipleEventPerEEG);
...
data = {'Comment' comment;...
    ...
    'Multiple events per EEG' isMultipleEventPerEEG; ...
    };
...
% --- Executes on button press in quantifyButton.
function quantifyButton_Callback(hObject, eventdata, handles)
% hObject    handle to quantifyButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

ScoreOneEvent(handles.SearchResultId);
