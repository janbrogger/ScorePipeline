function [timeSpanMinusGaps, recStart, eventTime] = ScoreGotoEvent(searchResultEventId)
    ...
    if isempty(existingPlot)
        ...
        recordingStartTime = ScoreQueryRun(['SELECT ' ...
                            'DATEPART(yyyy, Start) AS yy ' ...
                            ',DATEPART(mm, Start)   AS mm ' ...
                            ',DATEPART(dd, Start)   AS dd ' ...
                            ',DATEPART(HH, Start)   AS hh ' ...
                            ',DATEPART(mi, Start)   AS mi ' ...
                            ',DATEPART(ss, Start)   AS ss ' ...
                            ',DATEPART(ms, Start)   AS ms ' ...
                            'FROM SearchResult_Event  ' ...
                            'INNER JOIN  Event ON SearchResult_Event.EventId = Event.EventId ' ...
                            'INNER JOIN Recording ON Recording.RecordingId = Event.RecordingId ' ...
                            'WHERE SearchResult_Event.SearchResultEventId =  ' num2str(searchResultEventId)]);
        recordingStartTime = datetime(recordingStartTime.yy,recordingStartTime.mm,recordingStartTime.dd,recordingStartTime.hh,recordingStartTime.mi,recordingStartTime.ss,recordingStartTime.ms);
                
        EEG = evalin('base','EEG');
        %EEG.startDateTime did not exist for some reason. Quick fix in case
        %of multiple events
        isMultipleEventPerEEG = getappdata(0, 'isMultipleEventPerEEG');
        if(isMultipleEventPerEEG == 1)
            recStart = recordingStartTime;
        else    
