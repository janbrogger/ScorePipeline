...
signal = eegdata(chan,clicksample-500:clicksample+500);
x = 1:1:length(signal);
...
%in visual analysis (1cm per µV and 3cm per second).
searchdistance = 100;
...
title(num2str(chan));
hold on;
scatter(IEDspikepeak, IEDspikepeaky,7, 'red')
...
scatter(IEDspikestart, IEDspikestarty,9, 'blue')
scatter(IEDspikeend, IEDspikeendy,9,'blue')
...
if(hasSlow)
    str_ss = sprintf('%0.2f', exactareagfitsubtrapz/500);
